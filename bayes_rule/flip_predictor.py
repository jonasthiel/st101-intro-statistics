...
class FlipPredictor(object):
    ...
    def update(self,result):
        ...
        prob_head = self.pheads()
        if result == "H":
            self.probs = [coin * prob / prob_head for coin, prob in zip(self.coins, self.probs)]
        else:
            self.probs = [(1 - coin) * prob / (1 - prob_head) for coin, prob in zip(self.coins, self.probs)]
